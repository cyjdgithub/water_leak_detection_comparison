# fp_growth_cba_clean_readable_flow_fixed.py
import os
import random
...
if not flow_cols:
    raise RuntimeError("没有找到以 flow_ 开头的传感器列。")
log(f"📊 Using {len(flow_cols)} flow_ features.")
...
for col in flow_cols:
    ...
    df[col] = f"{col}_bin" + bins.astype(str)
...
if rules_df.empty:
    raise RuntimeError("❌no rule with class=... on right side")
...
rules_df.to_csv("rules_learned/fp_rules_cba_ready.csv", index=False)
...
log("✅ FP-Growth + CBA (readable, flow features) complete.")
