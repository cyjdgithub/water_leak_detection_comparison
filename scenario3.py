...
# === Select flow features only ===
print("🎯 Selecting flow_Link_xx columns...")
flow_cols = [col for col in df.columns if col.startswith("flow_Link_")]
print(f"Selected flow columns: {flow_cols}")
...
discretized_df.to_csv("discretization/scenario6_discretized.csv", index=False)
print("✅ Discretized data saved to 'scenario6_discretized.csv'.")
...
if rules:
    ...
    pd.DataFrame(rules).to_csv("rules_learned/scenario6_learned_rules.csv", index=False)
    pd.DataFrame([stats]).to_csv("rules_stats/scenario6_rule_stats.csv", index=False)
...
# 加载规则
rules_df = pd.read_csv("rules_learned/scenario6_learned_rules.csv")   # 改成 7/8 对应文件名
...
# 保存分类报告
with open("classfication_reports/scenario6_classification_report.txt", "w") as f:
    f.write(report)
